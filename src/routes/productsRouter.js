...
router.get("/", async (req, res) => {
  try {
    ...
    return res.status(201).json({ error: `Error al obtener los productos`, detalle: `${error.menssage}`})
  }
});
...
router.post("/login", async (req, res) => {
  ...
  try {//AGREGADO 2do probar q no se rompa el 1ero
  ...
  status: true,})
    res.setHeader("Content-Type", "application/json");
    return res.status(201).json({nuevoProducto});//AGREGADO
